...
d3.json('movementHierarchy.json', function (error, treeData) {
  ...
  d3.selectAll('.button').on('mousedown', tooldown).on('mouseup', toolup);
  ...
  function update(source, transition) {
    ...
    var nodeEnter = node
      ...
      .attr(
        'transform',
        'rotate(' + (source.x0 - 90) + ')translate(' + source.y0 + ')'
      )
      .on('click', click)
    ...
    var nodeUpdate = node
      ...
      .delay(
        transition
          ? function (d, i) {
              return (
                i * STAGGERN + Math.abs(d.depth - curNode.depth) * STAGGERD
              );
            }
          : 0
      )
    ...
    nodeUpdate.select('circle').attr('r', NODE_DIAMETER * reduceZ());
    // .style('fill', function(d) {
  ...
  function click(d) {
    // select node
    if (d3.event.defaultPrevented || d === curNode) {
      return;
    } // suppressed
    d3.event.preventDefault();
    selectNode(d);
  ...
  function keydown(key, shift) {
    ...
    switch (key) {
      ...
      case KEY_UP: // up arrow
        if (shift) {
          // move selection to previous child
          if (!curNode) {
            selectNode(root);
          } else if (curNode.parent) {
            parch = curNode.parent.children;
            selectNode(
              parch[(parch.indexOf(curNode) + parch.length - 1) % parch.length]
            );
          }
          update(curNode);
          return;
      ...
      case KEY_RIGHT: // right arrow
        if (shift) {
          // move selection to first/last child
          if (!curNode) {
            selectNode(root);
          } else {
            if (curNode.children) {
              selectNode(
                curNode.children[
                  d3.event.altKey ? curNode.children.length - 1 : 0
                ]
              );
            }
          }
